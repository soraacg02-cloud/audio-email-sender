...
# --- 設定頁面資訊 ---
st.set_page_config(page_title="音檔切割與寄送系統 (V10)", page_icon="📮", layout="wide")
st.title("📮 智慧音檔切割與寄送系統")
st.caption("🚀 核心 V10：新增「管理員後台」，可編輯與刪除歷史紀錄。")
...
with tab2:
    ...
    # 登入成功顯示後台
    if is_admin:
        st.success("✅ 登入成功！您可以編輯或刪除下方的歷史紀錄。")
        st.divider()
        
        st.subheader("📝 紀錄編輯器")
        st.info("操作說明：\n1. 點擊表格內的資料可直接修改。\n2. 選取左側的方框並按 Delete 鍵可標記刪除。\n3. **修改完畢後，請務必點擊下方「💾 儲存所有變更」按鈕。**")
        
        # 讀取目前的 CSV
        current_df = load_log()
        
        # 使用 data_editor 讓表格變為可編輯模式
        # num_rows="dynamic" 允許使用者增加或刪除行
        ...
        if st.button("💾 儲存所有變更 (Save Changes)", type="primary"):
            save_log(edited_df)
            st.session_state['mail_log_df'] = edited_df  # 更新快取
            st.success("🎉 資料庫已更新！(請切換回使用者模式查看結果)")
            st.rerun()
